<!DOCTYPE html>
<html lang="nl"> 
<head>
	 <meta charset="UTF-8">
	 <title>Land wijzigen verwerking</title>
	 <link rel="stylesheet" type="text/css" href="company.css">  
</head>

<body>
    <?php
        session_start();
        // Controleren of de gebruiker van het wijzigformulier afkomt
        if (!isset($_POST["cou_applyupdate"]))
        {
            header("Refresh: 4, url=cou-crud-get.php");
            echo "<h2>Je bent hier niet op de juiste manier gekomen!</h2>";
            exit();
        }

        // Formuliervelden ophalen en opschonen
        $cou_pk = test_input($_POST["cou_pk"]);
        
        // Het id moet nummeriek zijn
        if (!is_numeric($cou_pk))
        {
            header("Refresh: 9, url=cou-crud-get.php");
            echo "<h2>Je moet een nummer opgeven!!</h2>";
            exit();
        }

        // Controleren of de PK nog hetzelfde is (beveiliging tegen hacken)
        if (!isset($_SESSION["update_cou_pk"]) || $_SESSION["update_cou_pk"] <> $cou_pk)
        {
            header("Refresh: 4, url=index.php");
            echo "<h2>HACKER HACKER HACKER</h2>";
            echo "Je hebt geprobeerd de werking van het programma te wijzigen!";
            exit();
        }
        
        // Haal de velden op uit formulier
        $cou_name = test_input($_POST["cou_name"]);
        $cou_code = test_input($_POST["cou_code"]);

        // Controleer of naam ingevuld is en alleen letters/spaties bevat
        if (empty($cou_name) || !check_alfabet($cou_name))
        {
            header("Refresh: 4, url=cou-crud-upd.php");
            echo "<h2>De land naam moet ingevuld zijn (met alleen letters en spaties)</h2>";
            exit();
        }

        // Controleer of code ingevuld is en alleen letters/spaties bevat
        if (empty($cou_code) || !check_alfabet($cou_code))
        {
            header("Refresh: 4, url=cou-crud-upd.php");
            echo "<h2>De land code moet ingevuld zijn (met alleen letters en spaties)</h2>";
            exit();
        }

        // Controleer of land naam al bestaat (MAAR NIET met dezelfde ID)
        require_once "dbconnect.php";
        try 
        {
            $sQuery = "SELECT * FROM country WHERE name = :cou_name AND idcountry <> :cou_pk";
            $oStmt = $db->prepare($sQuery);
            $oStmt->bindValue(":cou_name", $cou_name);
            $oStmt->bindValue(":cou_pk", $cou_pk);
            $oStmt->execute();

            if ($oStmt->rowCount() > 0) 
            {
                header("Refresh: 4, url=cou-crud-upd.php");
                echo "<h2>Deze land naam bestaat al in de database!</h2>";
                exit();
            }
        } catch (PDOException $e) 
        {
            $sMsg = '<p> 
                        Regelnummer: ' . $e->getLine() . '<br /> 
                        Bestand: ' . $e->getFile() . '<br /> 
                        Foutmelding: ' . $e->getMessage() . ' 
                    </p>';
    
            trigger_error($sMsg);
            die();
        }

        // Controleer of land code al bestaat (MAAR NIET met dezelfde ID)
        try 
        {
            $sQuery = "SELECT * FROM country WHERE code = :cou_code AND idcountry <> :cou_pk";
            $oStmt = $db->prepare($sQuery);
            $oStmt->bindValue(":cou_code", $cou_code);
            $oStmt->bindValue(":cou_pk", $cou_pk);
            $oStmt->execute();

            if ($oStmt->rowCount() > 0) 
            {
                header("Refresh: 4, url=cou-crud-upd.php");
                echo "<h2>Deze land code bestaat al in de database!</h2>";
                exit();
            }
        } catch (PDOException $e) 
        {
            $sMsg = '<p> 
                        Regelnummer: ' . $e->getLine() . '<br /> 
                        Bestand: ' . $e->getFile() . '<br /> 
                        Foutmelding: ' . $e->getMessage() . ' 
                    </p>';
    
            trigger_error($sMsg);
            die();
        }

        // Pas na alle controles bouw je de header op

        try 
        {
            $sQuery = "UPDATE `country` SET `name`= :cou_name, `code`= :cou_code WHERE idcountry = :cou_pk";
            $oStmt = $db->prepare($sQuery);
            $oStmt->bindValue(":cou_name", $cou_name);
            $oStmt->bindValue(":cou_code", $cou_code);
            $oStmt->bindValue(":cou_pk", $cou_pk);
            $oStmt->execute();

            header("Refresh: 2, url=cou-crud-get.php");
            echo "<header class='spacebelowabove'>";
            echo "<h1>Land wijzigen</h1>";
            include "nav.html";
    	    echo "</header>";

            echo "<h2>Het land is bijgewerkt in de database!</h2>";

        } catch (PDOException $e) 
        {
            $sMsg = '<p> 
                        Regelnummer: ' . $e->getLine() . '<br /> 
                        Bestand: ' . $e->getFile() . '<br /> 
                        Foutmelding: ' . $e->getMessage() . ' 
                    </p>';
    
            trigger_error($sMsg);
            die();
        }

        // Unset session
        unset($_SESSION["update_cou_pk"]);

    ?>
    
    <?php
    // Hier komen alle functies te staan

    // test_input zorgt voor het opschonen van een veld in een formulier.
    function test_input($inpData)
    {
        $inpData = trim($inpData);
        $inpData = stripslashes($inpData);
        $inpData = htmlspecialchars($inpData);
        return $inpData;
    }

    // check_alfabet controleert of de input alleen uit letters en spaties bestaat.
    function check_alfabet($inpData)
    {
        if (preg_match("/^[a-zA-Z-' ]*$/",$inpData)) 
        {
            return true;
        }
        else
        {
            return false;
        }
    }

    ?>    

</body>
</html>
